...
export default function CallsDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'recent' | 'missed'>('recent');
              ...
              {recentCalls.map((call) => (
                <div key={call.id} className="flex items-center justify-between p-4 hover:bg-gray-50 rounded-lg transition-colors border border-gray-100">
                  ...
                  <div className="text-right">
                    ...
                    <p className={`text-xs ${call.status === 'completed' ? 'text-green-600' : call.status === 'missed' ? 'text-red-600' : 'text-gray-500'}`}>
                  ...
                  <div className="flex items-center space-x-3">
                    <div className="relative">
                      ...
                      <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${
                        contact.status === 'online' ? 'bg-green-500' : contact.status === 'busy' ? 'bg-red-500' : 'bg-gray-400'
                      }`}></div>
                    </div>
                    <div>
